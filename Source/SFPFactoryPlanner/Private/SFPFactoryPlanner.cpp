...
void FSFPFactoryPlannerModule::StartupModule()
{
	if (IsRunningDedicatedServer())
	{
		UE_LOG(
			...
			Display,
			TEXT("Pioneer Production Planner 1.5.2 loaded (world-max-fix-rc2) on dedicated server; client-only Slate input disabled"));
		return;
	}
	...
	UE_LOG(
		LogSFPFactoryPlanner,
		Display,
		TEXT("Pioneer Production Planner 1.5.2 loaded (world-max-fix-rc2); hotkey: %s"),
